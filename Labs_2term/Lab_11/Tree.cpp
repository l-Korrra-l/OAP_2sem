...
namespace btree      // бинарное дерево, не допускается дублирование ключей
{
	...
	//-------------------------------
	int Node::getLevel()          //подсчет текущего уровня
	{
		Node* rc = this;
		int q = 0;
	...
	//-------------------------------------------------------------------------------------------------------------------
	void Node::scanklr(void(*f)(void* n))
	{
		f(this->data);
		...
		if (this->left != NULL)
			this->left->scanklr(f);
		if (this->right != NULL)
			this->right->scanklr(f);
	}

	void Node::scanlkr(void(*f)(void* n))
	{
...
//---------------------------------------------------------------------------------------------------------------------------
	...
	int Object::maxd(Node* root)
	{
		if (!root) return 0;
		return max( maxd(root->left) , maxd(root->left) );
	}
	int Object::mind(Node* root)
	{
		if (!root) return 0;
		return min(mind(root->left), mind(root->left));
	}
	void Object::isbal(Node* root)
	{
		if (maxd(root) - mind(root) <= 1) cout << "Balanced" << endl;
		else cout << "Not balanced" << endl;
	}
//---------------------------------------------------------------------
	void  Object::scann(Node* root,int& k)  //Вывести вершины уровня
	{
		if (root->left != NULL)
			scann(root->left,k);
